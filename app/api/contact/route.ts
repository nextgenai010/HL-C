...
import { contactSchema as schema, type ContactData, MAX_FILES, MAX_FILE_SIZE, MAX_TOTAL_FILE_SIZE, ALLOWED_TYPES } from '@/lib/contact'
...
function ownerHtml(d: ContactData, files: { name: string; size: number }[]) {
  const filesRow = files.length
    ...
    : ''
  return `
<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#f7f6f3;color:#1a1b1d">
  <div style="border-top:3px solid #c8a96a;background:#fff;padding:28px">
    ...
    <h1 style="font-family:Arial,sans-serif;font-size:24px;line-height:1.25;margin:0 0 16px">${escapeHtml(d.type)}</h1>
...
function customerHtml(d: ContactData) {
  return `
<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#f7f6f3;color:#1a1b1d">
  <div style="border-top:3px solid #c8a96a;background:#fff;padding:28px">
    <p style="letter-spacing:.18em;text-transform:uppercase;font-size:12px;color:#c8a96a;margin:0 0 8px">✦ Tak for din besked</p>
    <h1 style="font-family:Arial,sans-serif;font-size:24px;line-height:1.3;margin:0 0 12px">Vi vender tilbage hurtigst muligt</h1>
    <p style="font-size:15px;line-height:1.6;color:#444;margin:0 0 16px">
      Hej ${escapeHtml(d.name.split(' ')[0])},<br><br>
      Tak fordi du skrev til os om <strong>${escapeHtml(d.type.toLowerCase())}</strong>. Vi har modtaget din forespørgsel og vender tilbage inden for 24 timer på hverdage.
    </p>
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      if (raw.reduce((sum, file) => sum + file.size, 0) > MAX_TOTAL_FILE_SIZE) {
        return NextResponse.json({ ok: false, error: 'files-too-large' }, { status: 400 })
      }
      for (const f of raw) {
  ...
  try {
    ...
    // The enquiry is received once the owner mail succeeds. A confirmation
    // failure must not encourage duplicate enquiries.
    try {
      ...
      if (confirmation.error) console.error('[contact] confirmation-mail fejlede', confirmation.error)
    } catch (error) {
      console.error('[contact] confirmation-mail fejlede', error)
    }
